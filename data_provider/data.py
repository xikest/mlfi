...
class Data:
    ...
    def _prepare_data(self, context:Context):
        ...
        context.prices = prices_volumes.loc[:,'Adj Close']
        context.volumes = prices_volumes.loc[:,'Volume']
        # pd.concat([price for price in gen_prices]).loc[:,'Adj Close'].unstack('ticker')
